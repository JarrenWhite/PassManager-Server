import os
import sys
import pytest
from flask import Flask
from typing import Dict, Any

sys.path.append(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "src"))

from api.api_password import password_bp
from services.service_password import ServicePassword


class TestStart():
    """Test cases for the Password Start API endpoint"""

    @pytest.fixture(autouse=True)
    def setup_teardown(self):
        self.app = Flask(__name__)
        self.app.register_blueprint(password_bp)
        self.client = self.app.test_client()

    def test_call_frontend(self, monkeypatch):
        """Should pass data through to frontend, and return given code"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "start", fake_service_handler)

        response = self.client.post(
            '/api/password/start',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 546
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {"false_json_key": "false_json_data"}

    def test_no_json_content(self, monkeypatch):
        """Should pass empty dict if no JSON or form data"""

        called = {"count": 0, "data": None}

        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 200
        monkeypatch.setattr(ServicePassword, "start", fake_service_handler)

        response = self.client.post('/api/password/start')

        assert response.status_code == 200
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {}

    def test_get_request(self, monkeypatch):
        """Should fail if GET request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "start", fake_service_handler)

        response = self.client.get(
            '/api/password/start',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_put_request(self, monkeypatch):
        """Should fail if PUT request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "start", fake_service_handler)

        response = self.client.put(
            '/api/password/start',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_delete_request(self, monkeypatch):
        """Should fail if DELETE request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "start", fake_service_handler)

        response = self.client.delete(
            '/api/password/start',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None


class TestAuth():
    """Test cases for the Password Auth API endpoint"""

    @pytest.fixture(autouse=True)
    def setup_teardown(self):
        self.app = Flask(__name__)
        self.app.register_blueprint(password_bp)
        self.client = self.app.test_client()

    def test_call_frontend(self, monkeypatch):
        """Should pass data through to frontend, and return given code"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "auth", fake_service_handler)

        response = self.client.post(
            '/api/password/auth',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 546
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {"false_json_key": "false_json_data"}

    def test_no_json_content(self, monkeypatch):
        """Should pass empty dict if no JSON or form data"""

        called = {"count": 0, "data": None}

        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 200
        monkeypatch.setattr(ServicePassword, "auth", fake_service_handler)

        response = self.client.post('/api/password/auth')

        assert response.status_code == 200
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {}

    def test_get_request(self, monkeypatch):
        """Should fail if GET request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "auth", fake_service_handler)

        response = self.client.get(
            '/api/password/auth',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_put_request(self, monkeypatch):
        """Should fail if PUT request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "auth", fake_service_handler)

        response = self.client.put(
            '/api/password/auth',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_delete_request(self, monkeypatch):
        """Should fail if DELETE request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "auth", fake_service_handler)

        response = self.client.delete(
            '/api/password/auth',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None


class TestComplete():
    """Test cases for the Password Complete API endpoint"""

    @pytest.fixture(autouse=True)
    def setup_teardown(self):
        self.app = Flask(__name__)
        self.app.register_blueprint(password_bp)
        self.client = self.app.test_client()

    def test_call_frontend(self, monkeypatch):
        """Should pass data through to frontend, and return given code"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "complete", fake_service_handler)

        response = self.client.post(
            '/api/password/complete',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 546
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {"false_json_key": "false_json_data"}

    def test_no_json_content(self, monkeypatch):
        """Should pass empty dict if no JSON or form data"""

        called = {"count": 0, "data": None}

        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 200
        monkeypatch.setattr(ServicePassword, "complete", fake_service_handler)

        response = self.client.post('/api/password/complete')

        assert response.status_code == 200
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {}

    def test_get_request(self, monkeypatch):
        """Should fail if GET request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "complete", fake_service_handler)

        response = self.client.get(
            '/api/password/complete',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_put_request(self, monkeypatch):
        """Should fail if PUT request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "complete", fake_service_handler)

        response = self.client.put(
            '/api/password/complete',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_delete_request(self, monkeypatch):
        """Should fail if DELETE request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "complete", fake_service_handler)

        response = self.client.delete(
            '/api/password/complete',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None


class TestAbort():
    """Test cases for the Password Abort API endpoint"""

    @pytest.fixture(autouse=True)
    def setup_teardown(self):
        self.app = Flask(__name__)
        self.app.register_blueprint(password_bp)
        self.client = self.app.test_client()

    def test_call_frontend(self, monkeypatch):
        """Should pass data through to frontend, and return given code"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "abort", fake_service_handler)

        response = self.client.post(
            '/api/password/abort',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 546
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {"false_json_key": "false_json_data"}

    def test_no_json_content(self, monkeypatch):
        """Should pass empty dict if no JSON or form data"""

        called = {"count": 0, "data": None}

        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 200
        monkeypatch.setattr(ServicePassword, "abort", fake_service_handler)

        response = self.client.post('/api/password/abort')

        assert response.status_code == 200
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {}

    def test_get_request(self, monkeypatch):
        """Should fail if GET request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "abort", fake_service_handler)

        response = self.client.get(
            '/api/password/abort',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_put_request(self, monkeypatch):
        """Should fail if PUT request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "abort", fake_service_handler)

        response = self.client.put(
            '/api/password/abort',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_delete_request(self, monkeypatch):
        """Should fail if DELETE request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "abort", fake_service_handler)

        response = self.client.delete(
            '/api/password/abort',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None


class TestGet():
    """Test cases for the Password Get API endpoint"""

    @pytest.fixture(autouse=True)
    def setup_teardown(self):
        self.app = Flask(__name__)
        self.app.register_blueprint(password_bp)
        self.client = self.app.test_client()

    def test_call_frontend(self, monkeypatch):
        """Should pass data through to frontend, and return given code"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "get", fake_service_handler)

        response = self.client.post(
            '/api/password/get',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 546
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {"false_json_key": "false_json_data"}

    def test_no_json_content(self, monkeypatch):
        """Should pass empty dict if no JSON or form data"""

        called = {"count": 0, "data": None}

        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 200
        monkeypatch.setattr(ServicePassword, "get", fake_service_handler)

        response = self.client.post('/api/password/get')

        assert response.status_code == 200
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {}

    def test_get_request(self, monkeypatch):
        """Should fail if GET request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "get", fake_service_handler)

        response = self.client.get(
            '/api/password/get',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_put_request(self, monkeypatch):
        """Should fail if PUT request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "get", fake_service_handler)

        response = self.client.put(
            '/api/password/get',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_delete_request(self, monkeypatch):
        """Should fail if DELETE request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "get", fake_service_handler)

        response = self.client.delete(
            '/api/password/get',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None


class TestUpdate():
    """Test cases for the Password Update API endpoint"""

    @pytest.fixture(autouse=True)
    def setup_teardown(self):
        self.app = Flask(__name__)
        self.app.register_blueprint(password_bp)
        self.client = self.app.test_client()

    def test_call_frontend(self, monkeypatch):
        """Should pass data through to frontend, and return given code"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "update", fake_service_handler)

        response = self.client.post(
            '/api/password/update',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 546
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {"false_json_key": "false_json_data"}

    def test_no_json_content(self, monkeypatch):
        """Should pass empty dict if no JSON or form data"""

        called = {"count": 0, "data": None}

        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 200
        monkeypatch.setattr(ServicePassword, "update", fake_service_handler)

        response = self.client.post('/api/password/update')

        assert response.status_code == 200
        assert response.is_json
        assert response.get_json() == {"false_key": "false_data"}

        assert called["count"] == 1
        assert called["data"] == {}

    def test_get_request(self, monkeypatch):
        """Should fail if GET request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "update", fake_service_handler)

        response = self.client.get(
            '/api/password/update',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_put_request(self, monkeypatch):
        """Should fail if PUT request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "update", fake_service_handler)

        response = self.client.put(
            '/api/password/update',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None

    def test_delete_request(self, monkeypatch):
        """Should fail if DELETE request used"""

        called = {"count": 0, "data": None}
        def fake_service_handler(data: Dict[str, Any]):
            called["count"] += 1
            called["data"] = data
            return {"false_key": "false_data"}, 546
        monkeypatch.setattr(ServicePassword, "update", fake_service_handler)

        response = self.client.delete(
            '/api/password/update',
            json={"false_json_key": "false_json_data"}
        )

        assert response.status_code == 405
        assert not response.is_json

        assert called["count"] == 0
        assert called["data"] == None


if __name__ == '__main__':
    pytest.main(['-v', __file__])
