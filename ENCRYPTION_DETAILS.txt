...
CRITICAL: Do not use a salt for the username's hash. This is used to identify the vault
          This does mean that the username is less secure, and is more susceptible to
          rainbow attacks. This is seen as unavoidable without enforcing random login IDs.
...
- KDFs: Argon2id with memory_cost=65536, time_cost=3, parallelism=1
...
Note: Some cryptographic libraries combine ciphertext and auth tag, others return separately.
      These must be separated before being sent to the server.
...
Process: Authentication tag is verified before decryption to ensure data integrity
